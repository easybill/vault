...
impl Crypto {
    ...
    pub fn key_decrypt(private_key: &PrivateKey, encrypted_key: &[u8]) -> Result<Vec<u8>> {
        ...
        let unencrypted_key = unencrypted_data[..size].to_vec();
...
mod test {
    ...
    fn test_key_round_trip() {
        ...
        let unencrypted_data = Crypto::key_decrypt(private_key, &encrypted_data).unwrap();

        assert_eq!(aes_key.as_slice(), unencrypted_data.as_slice());
    }
}
